...
export default function Login() {
  ...
  const handleGoogleSignUp = async () => {
    if (!licenseKey) {
      setError('Vui lòng nhập mã bản quyền.');
      return;
    }
    ...
    try {
      ...
      const { data: keyData, error: keyError } = await supabase.rpc('check_license_key_status', {
        key_text: licenseKey.toUpperCase().trim()
      });

      if (keyError || !keyData || !keyData.valid) {
        setError(keyData?.message || 'Mã bản quyền không hợp lệ hoặc đã được sử dụng.');
        setLoading(false);
        return;
      }
      ...
      <main className="flex-1 flex items-center justify-center p-4">
        <div className="bg-white rounded shadow-xl w-full max-w-md overflow-hidden border border-gray-200">
          ...
          <div className="p-5 sm:p-6">
            ...
            {isRegisterMode && (
              <div className="flex rounded border border-[#0b5394] mb-3 overflow-hidden text-sm">
                <button
                  type="button"
                  className={`flex-1 py-1.5 text-center font-semibold transition-colors ${
                    registerMethod === 'email'
                      ? 'bg-[#0b5394] text-white'
                      : 'bg-white text-[#0b5394] hover:bg-gray-50'
                  }`}
                  onClick={() => {
            ...
            <form className="space-y-3" onSubmit={isRegisterMode ? (registerMethod === 'google' ? (e) => { e.preventDefault(); handleGoogleSignUp(); } : handleSignUp) : handleLogin}>
              {isRegisterMode && registerMethod === 'email' && (
                <>
                  <div className="flex flex-col">
                    ...
                    <input
                      ...
                      name="fullName"
                      type="text"
                      required={isRegisterMode && registerMethod === 'email'}
              ...
              {(!isRegisterMode || (isRegisterMode && registerMethod === 'email')) && (
                <>
                  <div className="flex flex-col">
                    ...
                    <input
                      ...
                      type="email"
                      autoComplete="email"
                      required={!isRegisterMode || (isRegisterMode && registerMethod === 'email')}
                      className="w-full border border-gray-300 px-3 py-1.5 focus:outline-none focus:border-[#0b5394] focus:ring-1 focus:ring-[#0b5394] rounded text-sm transition-colors"
                      value={email}
              ...
              <div className="pt-1">
                {isRegisterMode && registerMethod === 'google' ? (
                  ...
                  <>
                    ...
                    >
                      {loading ? 'Đang xử lý...' : (isRegisterMode ? 'Đăng ký bằng Email & Mật khẩu' : 'Đăng nhập')}
                    </button>
                      ...
                      <>
                        <div className="relative my-3">
                          ...
                          <div className="relative flex justify-center text-[11px] uppercase">
                            <span className="bg-white px-2 text-gray-500 font-semibold text-gray-500">Hoặc đăng nhập bằng</span>
                          </div>
                        </div>
                        ...
                        <button
                          ...
                          disabled={loading}
                          className="w-full flex items-center justify-center gap-2 border border-gray-300 bg-white hover:bg-gray-50 text-gray-700 font-semibold py-2 px-4 rounded shadow-sm transition-colors text-sm"
                        >
                          <svg className="w-4 h-4 mr-1" viewBox="0 0 24 24" width="16" height="16">
                            <path
                              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                    ...
                    {isRegisterMode && (
                      <>
                        <div className="relative my-3">
                          ...
                          <div className="relative flex justify-center text-[11px] uppercase">
                            <span className="bg-white px-2 text-gray-500 font-medium">Hoặc đăng ký bằng</span>
                          </div>
                        </div>
